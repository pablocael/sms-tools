...
def computeODF(inputFile, window, M, N, H):
    ...
    remainderHopSamples = len(x) % H
    if remainderHopSamples != 0:
        pad = np.zeros(len(x) + H-remainderHopSamples)
        pad[:len(x)] = x
        x = pad
    ...
    mX = np.power(10, mX/20)
    low_band_index = (binFreq > 0) & (binFreq < 3000)
    high_band_index = (binFreq > 3000) & (binFreq < 10000)

    # Extract the bands using the indices
    low_band = np.power(mX[:, low_band_index], 2)
    high_band = np.power(mX[:, high_band_index], 2)
    
    engEnv = np.zeros((low_band.shape[0],2))
    low = np.sum(low_band, axis=1)
    low[low < eps] = eps
    high = np.sum(high_band, axis=1)
    high[high < eps] = eps
    engEnv[:, 0] = 10*np.log10(low)
    engEnv[:, 1] = 10*np.log10(high)
    odf = np.zeros_like(engEnv)
    for i in range(1, odf.shape[0]):
